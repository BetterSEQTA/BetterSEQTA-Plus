<script lang="ts">
  ...
  import type { Theme } from '@/interface/types/Theme'; // Type definition for theme
  import ThemeCard from './ThemeCard.svelte'; // Importing the ThemeCard component

  // Props for the component: list of themes, search term, and function to set the selected theme
  let { themes, searchTerm, setDisplayTheme } = $props<{ themes: Theme[]; searchTerm: string, setDisplayTheme: (theme: Theme) => void }>();

  // Filter themes based on the search term in name or description
...
<div class="relative" >
  <!-- Grid to display the theme cards -->
  <div class="grid grid-cols-1 gap-4 py-12 mx-auto sm:grid-cols-2 lg:grid-cols-3">
    <!-- Loop through filteredThemes and display each theme using the ThemeCard component -->
    {#each filteredThemes as theme (theme.id)}
      <ThemeCard theme={theme} onClick={() => setDisplayTheme(theme)} />
    {/each}
  
    <!-- Show "Got a Theme Idea?" section if there are filtered themes -->
    {#if filteredThemes.length !== 0}
      <a href="https://betterseqta.gitbook.io/betterseqta-docs" class='w-full cursor-pointer'>
        <div class="bg-zinc-50 h-48 w-full transition-all hover:scale-105 duration-500 relative justify-center items-center group group/card flex flex-col hover:shadow-2xl dark:hover:shadow-white/[0.1] hover:shadow-white/[0.8] dark:bg-zinc-800 dark:border-white/[0.1] rounded-xl overflow-clip border">
          <div class="text-2xl font-IconFamily">{'\uecb3'}</div> <!-- Icon for theme idea section -->
  ...
  <!-- Show "No themes found" section if there are no filtered themes -->
  {#if filteredThemes.length === 0}
