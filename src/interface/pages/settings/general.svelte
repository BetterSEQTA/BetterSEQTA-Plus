      ...
      <!-- Display enable toggle if disableToggle is true -->
        {#if (plugin as any).disableToggle}
          <div class="flex justify-between items-center px-4 py-3">
            ...
            <div>
              <Switch
                state={pluginSettingsValues[plugin.pluginId]?.enabled ?? true} <!-- Enable toggle switch for plugin -->
                onChange={(value) => updatePluginSetting(plugin.pluginId, 'enabled', value)} <!-- Update plugin setting on change -->
              />
            </div>
          </div>
        {/if}
        ...
        {#if !((plugin as any).disableToggle) || (pluginSettingsValues[plugin.pluginId]?.enabled ?? true)}
          {#each Object.entries(plugin.settings)
