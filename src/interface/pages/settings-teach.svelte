...
<!-- Teach-specific settings page with larger size and Teach-optimized layout -->
<div
  class="w-[420px] no-scrollbar shadow-2xl {$settingsState.DarkMode
    ? 'dark'
    : ''} {standalone ? 'h-[700px]' : 'h-full rounded-xl'} overflow-clip"
>
  <div
    class="flex relative flex-col gap-2 h-full overflow-clip bg-white dark:bg-zinc-800 dark:text-white"
  >
    <div
      class="grid place-items-center border-b border-b-zinc-200/40 dark:border-b-zinc-700/40 p-4"
    >
      ...
      {#if !standalone}
        <button
          onclick={openAbout}
          class="absolute top-1 right-[166px] w-8 h-8 text-lg rounded-xl font-IconFamily bg-zinc-100 dark:bg-zinc-700"
        >
          {"\ueb73"}
        </button>
