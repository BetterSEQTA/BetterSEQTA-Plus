...
import { isSEQTALearnSync } from "../platformDetection";

// Learn-only changelog popup
export function OpenWhatsNewPopup() {
  // Only show for Learn platform
  if (!isSEQTALearnSync()) {
    console.debug("[BetterSEQTA+] OpenWhatsNewPopup called but not on Learn platform");
    return;
  }
...
// Teach-only changelog popup
export function OpenWhatsNewPopupTeach() {
  const header = stringToHTML(
    /* html */
    `<div class="whatsnewHeader">
        <h1>What's New in Teach</h1>
        <p>BetterSEQTA+ V${browser.runtime.getManifest().version}</p>
      </div>`,
  ).firstChild as HTMLElement;
  ...
  const text = stringToHTML(/* html */ `
    <div class="whatsnewTextContainer" style="height: 50%;overflow-y: auto;">
      ...
      <h1>3.4.13 - Bug Fixes & Styling Improvements</h1>
      <li>Started adding support for the Teach platform</li>
      <li>Added the changelog popup for the Teach platform</li>
