class ReactFiber {
  ...
  private async sendMessage(action: string, payload: any = {}): Promise<any> {
    return new Promise((resolve, _) => {
      ...
      const message = {
        type: "reactFiberRequest", // Type of message
        selector: this.selector, // The selector of the React component
        action, // Action to perform (e.g., getState, setState)
        payload, // Data associated with the action
        debug: this.debug, // Include debug flag if enabled
