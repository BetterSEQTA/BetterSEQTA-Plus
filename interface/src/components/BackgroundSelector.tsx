...
const BACKGROUND_SIZE = 16;
const DELETE_BUTTON_SIZE = 6;
  ...
  return (
    ...
    <div className="relative">
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className={`relative w-${BACKGROUND_SIZE} h-${BACKGROUND_SIZE} overflow-hidden transition rounded-xl bg-zinc-100 dark:bg-zinc-900`}>
        ...
        {backgrounds.filter(bg => bg.type === 'image').map(bg => (
          <div key={bg.id}
            ...
            className={`relative w-${BACKGROUND_SIZE} h-${BACKGROUND_SIZE} cursor-pointer rounded-xl transition ring dark:ring-white ring-zinc-300 ${isEditMode ? 'animate-shake' : ''} ${selectedBackground === bg.id && selectedType === "background" ? 'dark:ring-2 ring-4' : 'ring-0'}`}>
            {isEditMode && (
              <div className={`absolute top-0 right-0 z-10 flex w-${DELETE_BUTTON_SIZE} h-${DELETE_BUTTON_SIZE} p-2 text-white translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full place-items-center`}
        ...
        {backgrounds.concat(presetBackgrounds as Background[]).filter(bg => bg.type === 'image' && bg.isPreset && !bg.isDownloaded && !downloadedPresetIds.includes(bg.id)).map(bg => (
          <div key={bg.id}
            ...
            className={`relative w-${BACKGROUND_SIZE} h-${BACKGROUND_SIZE} transition cursor-pointer rounded-xl duration-300 ${ isEditMode ? 'opacity-0 pointer-events-none hidden' : 'opacity-100'}`}>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        {backgrounds.filter(bg => bg.type === 'video').map(bg => (
          <div key={bg.id} onClick={() => selectBackground(bg.id)} className={`relative w-${BACKGROUND_SIZE} h-${BACKGROUND_SIZE} cursor-pointer rounded-xl transition ring dark:ring-white ring-zinc-300 ${isEditMode ? 'animate-shake' : ''} ${selectedBackground === bg.id && selectedType === "background" ? 'dark:ring-2 ring-4' : 'ring-0'}`}>
            {isEditMode && (
              <div className={`absolute top-0 right-0 z-10 flex w-${DELETE_BUTTON_SIZE} h-${DELETE_BUTTON_SIZE} p-2 text-white translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full place-items-center`}
                  onClick={() => deleteBackground(bg.id)}>
                <div className="w-4 h-0.5 bg-white"></div>
